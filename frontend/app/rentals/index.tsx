    ...
    <View className="flex-1 bg-[#1a1a1a]">
      ...
      <ScrollView className="p-6">
        <Text className="text-gray-400 mb-6">
          Choose equipment for your next shoot.
        </Text>

        {ASSETS.map((asset) => {
          const categoryName = CATEGORIES.find(
            (c) => c.categoryId === asset.categoryId,
          )?.categoryName;
          ...
          return (
            ...
            >
              ...
              <View className="flex-1 p-4">
                <Text className="text-xs text-gray-400 uppercase mb-1 tracking-wider">
                  {categoryName}
                </Text>
                <Text className="text-lg font-bold text-white mb-2">
                  {asset.modelName}
                </Text>
                <Text className="text-[#FF8C42] font-semibold mb-2">
                  ₫{asset.dailyRate.toLocaleString()}{" "}
                  <Text className="text-gray-500 text-xs">/day</Text>
                </Text>

                <View
                  className={`self-start px-3 py-1 rounded-full ${isRented ? "bg-red-500/20" : "bg-green-500/20"}`}
                >
                  <Text
                    className={`text-xs font-semibold ${isRented ? "text-red-500" : "text-green-500"}`}
                  >
                    {asset.status}
                  </Text>
                </View>
