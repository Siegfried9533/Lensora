...
import com.camerashop.entity.Rental.RentalStatus;
...
public interface RentalRepository extends JpaRepository<Rental, String> {
    ...
    /** Unpaid holds older than the cutoff — released by the scheduled job so the asset frees up. */
    @Query("SELECT r FROM Rental r WHERE r.status = :status AND r.createdAt < :cutoff")
    List<Rental> findExpiredHolds(@Param("status") RentalStatus status, @Param("cutoff") LocalDateTime cutoff);
